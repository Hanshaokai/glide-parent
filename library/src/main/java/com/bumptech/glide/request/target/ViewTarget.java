...
public abstract class ViewTarget<T extends View, Z> extends BaseTarget<Z> {
    ...
    private static class SizeDeterminer {
        ...
        void checkCurrentDimens() {
            ...
            if (!isViewStateAndSizeValid(currentWidth, currentHeight)) { // 再次判断
                return;// 扔没绘制好 停止回调 继续等待
            }
            notifyCbs(currentWidth, currentHeight);
            clearCallbacksAndListener();// 清除监听 清除缓存
        }

        void getSize(SizeReadyCallback cb) {
            int currentWidth = getTargetWidth();// 得到imagview 的宽度
            int currentHeight = getTargetHeight();// 得到imageView 的高度  获得高宽 来确定view 是否绘制完
            ...
            if (layoutListener == null) {
                ViewTreeObserver observer = view.getViewTreeObserver();// 得到观察者
                layoutListener = new SizeDeterminerLayoutListener(this);// 放置监听
        ...
        private boolean isViewStateValid() {
            ...
            // non-null layout params and a non-zero layout width and height. 没有外层控件会获得空对象 这种情况无法确定是否绘制
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
                return view.isLaidOut();// 是否经过过layout  这种情况仍包含不了全部
            }
            return !view.isLayoutRequested();// 没有请求加载 就是加载了
        }
        ...
        private static class SizeDeterminerLayoutListener implements ViewTreeObserver
            ...
            SizeDeterminerLayoutListener(SizeDeterminer sizeDeterminer) { // 静态内部类持有 弱引用   访问对象的成员方法 和对象
            ...
            public boolean onPreDraw() {
                ...
                if (sizeDeterminer != null) {// 绘制完成 回调
