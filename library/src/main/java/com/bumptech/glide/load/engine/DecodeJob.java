...
class DecodeJob<R> implements DataFetcherGenerator.FetcherReadyCallback,
    ...
    private void runWrapped() {
        switch (runReason) {
            case INITIALIZE://先初始化
                stage = getNextStage(Stage.INITIALIZE);
                //根据stage 选择从哪里获得数据getNextGenerator
                currentGenerator = getNextGenerator();
                // return new SourceGenerator(decodeHelper, this);
                runGenerators();//进入此方法找网络请求
                break;
            // 再回来 走这里
...
//currentGenerator.startNext()  进入此方法
    ...
    private void notifyComplete(Resource<R> resource, DataSource dataSource) {
        setNotifiedOrThrow();
        callback.onResourceReady(resource, dataSource);// 继续向上回调  到EngineJob
    }
    ...
    public void onDataFetcherReady(Key sourceKey, Object data, DataFetcher<?> fetcher,
        ...
        if (Thread.currentThread() != currentThread) {//线程判断  如果不在子线程了 就在此开启子线程 进入子线程解码  仍然走decodeFromRetrievedData 方法
            ...
            decodeFromRetrievedData();//解码获取的数据
        }
    }
