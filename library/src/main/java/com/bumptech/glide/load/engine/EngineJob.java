...
class EngineJob<R> implements DecodeJob.Callback<R>,
    ...
    private static final Handler MAIN_THREAD_HANDLER =
            new Handler(Looper.getMainLooper(), new MainThreadCallback()); // 往主线程中创建handler
    ...
    public void start(DecodeJob<R> decodeJob) {
        ...
        executor.execute(decodeJob); // 将线程放到线程池里执行  execute 方法执行此线程
    }
    ...
    void handleResultOnMainThread() {
        ...
        engineResource = engineResourceFactory.build(resource, isCacheable);// 包装Bitmap 的Resource的封装类  继续封装成EngineResource
        hasResource = true;
    ...
    public void onResourceReady(Resource<R> resource, DataSource dataSource) {
        this.resource = resource;//Bitmap 的Resource的封装类 获得
    ...
    void handleExceptionOnMainThread() {
        ...
        for (ResourceCallback cb : cbs) {
            if (!isInIgnoredCallbacks(cb)) {
                cb.onLoadFailed(exception); // 通知 集合里所有的对象 进入所有对象的实现方法 中进行处理  如 SinggleRequest
            }
        }
        ...
        public boolean handleMessage(Message message) {
            ...
            switch (message.what) {
                case MSG_COMPLETE:
                    ...
                    break;
                case MSG_EXCEPTION: // 失败走这里  回到主线程
