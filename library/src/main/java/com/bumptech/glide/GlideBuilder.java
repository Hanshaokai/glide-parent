...
public final class GlideBuilder {
    ...
    public Glide build(Context context) {
        ...
        if (sourceExecutor == null) {
            sourceExecutor = GlideExecutor.newSourceExecutor();// 初始化网络线程池+
        }

        if (diskCacheExecutor == null) {
            diskCacheExecutor = GlideExecutor.newDiskCacheExecutor();// 初始化 缓存线程池
        }

        if (memorySizeCalculator == null) {
            memorySizeCalculator = new MemorySizeCalculator.Builder(context).build(); // 构造内存大小计算器
        }

        if (connectivityMonitorFactory == null) {
            connectivityMonitorFactory = new DefaultConnectivityMonitorFactory();//网络连接管理工厂
        }

        if (bitmapPool == null) {
            int size = memorySizeCalculator.getBitmapPoolSize();// 图片池
        ...
        if (diskCacheFactory == null) {
            diskCacheFactory = new InternalCacheDiskCacheFactory(context);// 内部缓存 硬盘存储工厂
        }

        if (engine == null) {
            engine = new Engine(memoryCache, diskCacheFactory, diskCacheExecutor, sourceExecutor,
                    GlideExecutor.newUnlimitedSourceExecutor());// 初始化引擎
        }

        RequestManagerRetriever requestManagerRetriever = new RequestManagerRetriever(
                requestManagerFactory);//  从modul 产生器 中获得的
